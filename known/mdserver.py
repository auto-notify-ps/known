...
def HOME_PAGE(content, style=""): return """
<html>
    <head>
        <meta charset="UTF-8">
        ...
        <style type="text/css">mjx-container[jax="CHTML"][display="true"] { text-align: left; }</style>
        <style type="text/css">
        """ + style + """
        </style>
    </head>
    ...
    <br>
    <!-- ---------------------------------------------------------->
    <div class="board_content">""" + content + f"""
    </div>
    <!-- ---------------------------------------------------------->
    <br>
    <!-- ---------------------------------------------------------->
...
def nb2html(source_notebook, html_title=None, 
    ...
    if html_title is None:
        ...
        if html_title.lower().endswith(EXT): html_title = html_title[:-6]
    ...
    page = HOME_PAGE(markdown.markdown(md_text, extensions=['fenced_code']))
    page = render_template_string(page)
...
if not os.path.isfile(HOME_PATH):
    try: 
        with open(HOME_PATH, 'w') as f: f.write( f"\n# {os.path.basename(BASE)}\n")
    ...
    if not ((os.path.isfile(requested)) and (not os.path.relpath(requested, base).startswith(base))): return abort(404)
        ...
        if tosend: 
            ...
            if refresh: return redirect(url_for('route_home', query=query))
                ...
                if download: return send_file(requested, as_attachment=True) #<--- downloading 
